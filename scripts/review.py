...
Uses GitHub Models (gpt-4o) via GITHUB_TOKEN — no separate API key needed.
...
def main():
    if not GH_TOKEN or PR_NUMBER == 0 or not REPO:
        print("ERROR: GH_TOKEN / PR_NUMBER / REPO are required.", file=sys.stderr)
        sys.exit(1)

    print(f"Reviewing PR #{PR_NUMBER} in {REPO} using GitHub Models ({MODEL})")
    pr       = get_pr_info()
    pr_title = pr["title"]
